...
الواجهة الأمامية: المستخدم يضغط على رابط "الدخول كضيف" في `login.html`.

إنشاء جلسة مؤقتة:

يتم استدعاء دالة `handleGuestLogin()` في `js/login-page.js`.

تقوم الدالة بإنشاء كائن مستخدم ضيف (`{ is_guest: true, user_key: 'guest_user' }`) وحفظه في `localStorage`.

تقييد الوظائف:

عندما يتم اكتشاف أن `user.is_guest` هو `true`، يتم تعطيل بعض الميزات مثل إتمام الشراء أو حفظ السلة بشكل دائم.

يتم تخطي إعداد إشعارات FCM للمستخدم الضيف لتجنب الطلبات غير الضرورية.
...
### 8. تدفقات إدارة المنتجات (للبائع)

هذا القسم يشرح بالتفصيل كيف يقوم البائع بإدارة منتجاته من خلال عرضها وتعديلها وحذفها.

#### 8.1. عرض منتجاتي (View My Products)

1.  **البداية (الواجهة الأمامية):** 
    *   بعد تسجيل الدخول، يضغط البائع على زر "عرض منتجاتي" في `login.html`.
    *   يتم استدعاء دالة `showMyProducts(userKey)` في `js/login-page.js`.

2.  **جلب البيانات:**
    *   `showMyProducts` تستدعي `getProductsByUser(userKey)` من `js/turo.js`.
    *   `turo.js` يرسل طلب `GET` إلى `api/products.js` مع `user_key` الخاص بالبائع.

3.  **المعالجة (الواجهة الخلفية):**
    *   `api/products.js` يستقبل الطلب وينفذ استعلام `SELECT` في قاعدة بيانات Turso لجلب جميع المنتجات التي يملكها هذا البائع.

4.  **النتيجة (الواجهة الأمامية):**
    *   تستقبل دالة `showMyProducts` قائمة المنتجات.
    *   تقوم ببناء "بطاقة منتج" (`product-card`) لكل منتج داخل نافذة منبثقة، تحتوي على التفاصيل والصور.
    *   يتم إضافة زري "تعديل" و"إزالة" لكل بطاقة، مع تخزين بيانات المنتج الكاملة داخل خاصية `data-product` في كل زر.
    *   يتم توفير فلاتر لتصفية المنتجات حسب الفئة الرئيسية والفرعية، بالإضافة إلى حقل بحث بالاسم.

 
#### 8.2. تعديل منتج موجود (Edit Product)

1.  **البداية (الواجهة الأمامية):**
    *   يضغط البائع على زر "تعديل" في بطاقة المنتج.
    *   يتم قراءة بيانات المنتج من `data-product` وفتح نموذج `pages/addProduct.html` في "وضع التعديل".
    *   يتم ملء النموذج بالبيانات الحالية للمنتج وصوره.

2.  **التدفق المحسّن:**
    *   يتم إخفاء نافذة "منتجاتي" مؤقتًا بدلاً من إغلاقها.
    *   عند إغلاق نافذة التعديل، يتم إعادة إظهار نافذة "منتجاتي" تلقائيًا، مما يوفر تجربة مستخدم سلسة.
2.  **إدارة الصور والتعديلات:**
    *   يقوم البائع بإجراء التعديلات المطلوبة (نصوص، أسعار، صور).
    *   عند الضغط على "حفظ التعديلات"، يقوم السكريبت في `addProduct.html` بالآتي:
        *   **حذف الصور القديمة:** يستدعي `deleteFile2cf()` من `cloudFileManager.js` لحذف أي صور أزالها البائع من Cloudflare R2.
        *   **رفع الصور الجديدة:** يستدعي `uploadFile2cf()` لضغط ورفع أي صور جديدة إلى Cloudflare R2.

3.  **تحديث قاعدة البيانات:**
    *   بعد إدارة الصور، يتم تجميع بيانات المنتج المحدثة والقائمة النهائية لأسماء الصور.
    *   يتم استدعاء `updateProduct(productData)` من `js/turo.js`، الذي يرسل طلب `PUT` إلى `api/products.js`.

4.  **النتيجة:** الواجهة الخلفية تحدث سجل المنتج، والواجهة الأمامية تغلق نافذة التعديل وتُحدّث عرض "منتجاتي" تلقائيًا.

#### 8.3. حذف منتج (Delete Product)

1.  **البداية (الواجهة الأمامية):**
    *   يضغط البائع على زر "إزالة".
    *   تظهر رسالة تأكيد (`SweetAlert2`).

2.  **تنفيذ الحذف:**
    *   عند التأكيد، تستدعي دالة `deleteProductAndImages` في `js/login-page.js`.
    *   **أولاً (حذف الملفات):** يتم حذف **جميع** صور المنتج من Cloudflare R2 عبر استدعاء `deleteFile2cf()` لكل صورة.
    *   **ثانياً (حذف البيانات):** يتم استدعاء `deleteProduct(productKey)` من `js/turo.js`، الذي يرسل طلب `DELETE` إلى `api/products.js`.

3.  **المعالجة (الواجهة الخلفية):** `api/products.js` ينفذ أمر `DELETE FROM marketplace_products` لحذف سجل المنتج نهائيًا.

4.  **النتيجة:** بعد نجاح الحذف، تُعرض رسالة نجاح ويتم تحديث عرض "منتجاتي" لإزالة بطاقة المنتج المحذوف.

### 9. تدفق نظام الإعلانات (Advertisement System Flow)

هذا القسم يشرح الدورة الكاملة لنظام الإعلانات، من إدارته بواسطة المسؤول إلى عرضه للمستخدم النهائي في الصفحة الرئيسية. يعتمد النظام على تصميم ذكي يوازن بين عرض أحدث محتوى وتقديم أسرع تجربة تحميل ممكنة.

#### 9.1. عرض الإعلانات للمستخدم (الواجهة الأمامية - `adverModule.js`)

تتم عملية عرض شريط الإعلانات المتحرك في الصفحة الرئيسية (`index.html`) عبر آلية تخزين مؤقت ذكية لتقليل طلبات الشبكة وتسريع التحميل.

1.  **البداية والتحقق السريع:**
    *   عند تحميل الصفحة، يتم استدعاء `initAdverModule()`.
    *   **الفحص الأول:** يتحقق الموديول من وجود "وقت آخر فحص" (`adver_last_check_timestamp`) في `localStorage`.
    *   **إذا مر أقل من 24 ساعة:** يتم عرض الصور المحفوظة محلياً **فوراً** دون أي اتصال بالشبكة. هذا يجعل تحميل الإعلانات شبه فوري للمستخدمين العائدين.

2.  **التحقق من التحديثات من الخادم:**
    *   إذا مر أكثر من 24 ساعة على آخر فحص، يتصل الموديول بـ `api/updates.js` لجلب "آخر تاريخ تحديث" (`serverTimestamp`).
    *   **إذا كان التاريخ متطابقاً** مع التاريخ المحفوظ محلياً، يتم عرض الصور من الذاكرة المؤقتة وتحديث "وقت آخر فحص" إلى الآن.

3.  **جلب الصور الجديدة من الشبكة:**
    *   تتم هذه الخطوة فقط إذا كان تاريخ التحديث على الخادم مختلفاً أو لا توجد صور مخزنة.
    *   يتصل الموديول مباشرة بخدمة تخزين الملفات (Cloudflare R2) ويبحث عن الصور (`pic1.webp`, `pic2.webp`, ...) بالتوازي.
    *   بعد جلب الصور الجديدة، يتم حفظها في `localStorage` مع "تاريخ التحديث" الجديد.

4.  **بناء الشريط الإعلاني:**
    *   تستخدم دالة `buildSlider` الصور التي تم جلبها لبناء شريط إعلاني متحرك وتفاعلي يعمل تلقائياً.

#### 9.2. إدارة الإعلانات (لوحة تحكم المسؤول - `Advertiesment.html`)

يتمكن المسؤول من إدارة الإعلانات عبر نافذة منبثقة ديناميكية وآمنة.

1.  **فتح نافذة الإدارة:**
    *   في صفحة `admin.html`، يضغط المسؤول على "إدارة الإعلانات".
    *   يتم تحميل محتوى `pages/Advertiesment.html` داخل نافذة منبثقة.

2.  **تحميل الصور الحالية:**
    *   يتم استدعاء `loadExistingAdverImages()` التي تتصل بـ Cloudflare R2 لجلب وعرض الصور الإعلانية الموجودة حالياً.
    *   يتم تخزين أسماء الصور الأصلية (`originalImageNames`) لاستخدامها لاحقاً في عملية المزامنة.

3.  **التفاعل مع النموذج:**
    *   **إضافة:** يمكن للمسؤول إضافة صور جديدة، حيث يتم ضغطها وتحويلها إلى `WebP` في المتصفح.
    *   **حذف:** يمكن حذف الصور من الواجهة.
    *   **إعادة ترتيب:** يمكن سحب وإفلات الصور لتغيير ترتيب عرضها.

#### 9.3. نشر وتحديث الإعلانات (المزامنة مع الخادم)

عندما يضغط المسؤول على "نشر الإعلان الآن"، تبدأ عملية مزامنة ذكية من عدة مراحل لضمان عدم فقدان البيانات.

1.  **تخطيط المزامنة:**
    *   يقوم السكريبت بمقارنة الحالة النهائية للصور في الواجهة مع قائمة الصور الأصلية.
    *   يتم تحديد ثلاث قوائم:
        *   **صور للحذف (`imagesToDelete`):** الصور التي تمت إزالتها من الواجهة أو الخانات التي أصبحت فارغة.
        *   **صور للرفع (`newUploads`):** الصور الجديدة التي أضافها المسؤول.
        *   **صور لإعادة الترتيب (`reorderItems`):** الصور التي تغير ترتيبها.

2.  **التنفيذ (بالترتيب لضمان الأمان):**
    *   **المرحلة الأولى (الحذف):** يتم إرسال طلبات `DELETE` إلى Cloudflare Worker (`file-manager.js`) لحذف جميع الصور في قائمة `imagesToDelete`. هذا يضمن إفراغ الأماكن القديمة أولاً.
    *   **المرحلة الثانية (الرفع):** يتم إرسال طلبات `POST` لرفع الصور الجديدة فقط، وتتم تسميتها بالترتيب الجديد (`pic1.webp`, `pic2.webp`, ...).
    *   **المرحلة الثالثة (إعادة الترتيب):** تتم هذه العملية بأمان على ثلاث خطوات فرعية لتجنب الكتابة فوق ملف موجود عن طريق الخطأ:
        1.  **النسخ الاحتياطي:** نسخ كل ملف سيتم تغيير مكانه إلى اسم مؤقت (مثلاً: `pic3.webp` يُنسخ إلى `temp_pic3.webp`).
        2.  **النشر النهائي:** نسخ الملفات من أسمائها المؤقتة إلى أسمائها النهائية الجديدة (مثلاً: `temp_pic3.webp` يُنسخ إلى `pic1.webp`).
        3.  **التنظيف:** حذف كل من الملفات المؤقتة (`temp_...`) والملفات الأصلية التي تم نسخها.

3.  **تسجيل التحديث:**
    *   بعد نجاح كل عمليات الملفات، يتم استدعاء `addUpdate()` التي ترسل طلباً إلى `api/updates.js`.
    *   يقوم `api/updates.js` بتحديث حقل `datetime` في قاعدة البيانات إلى الوقت الحالي. هذا التاريخ هو الذي يستخدمه `adverModule.js` في المرة القادمة لمعرفة ما إذا كان يجب جلب الصور الجديدة.

4.  **النتيجة النهائية:** تُعرض رسالة نجاح للمسؤول، ويتم إعادة تحميل نموذج الإدارة ليعكس الحالة الجديدة للصور على الخادم.
...
يتم التحقق أولاً من أن المستخدم مسجل دخوله وليس ضيفًا. إذا لم يكن، تظهر رسالة تطلب منه تسجيل الدخول.

تُستدعى دالة addToCart() من js/cart.js.

addToCart() تستقبل كائن المنتج الذي يتضمن seller_key (وهو user_key الخاص بالبائع)، ثم تقوم بجلب السلة الحالية من LocalStorage باستخدام مفتاح مربوط بـ `user_key` الخاص بالمستخدم (`cart_user123`).
...
### 11. تدفق نظام الإشعارات (Firebase Cloud Messaging) - شرح دقيق
...
*   **متى؟**: تتم هذه العملية **فورًا** بعد نجاح تسجيل دخول المستخدم (وليس الضيف)، وتحديدًا عند استدعاء دالة `checkLoginStatus` في `js/auth.js`.
*   **كيف؟**:
...
*   **الإرسال (من الخادم)**:
    ...
    *   يستخدم `api/send-notification.js` مكتبة `firebase-admin` لإرسال رسالة بحمولة `data` (وليس `notification`) إلى التوكن المحدد. هذا يضمن أن عامل الخدمة في الخلفية (`firebase-messaging-sw.js`) يستقبل الإشعار دائمًا ويتولى عرضه.
*   **الاستقبال (في المتصفح)**:
    *   **إذا كان التطبيق مغلقًا**: يتولى ملف `firebase-messaging-sw.js` (Service Worker) مهمة استقبال الإشعار وعرضه.
    *   **إذا كان التطبيق مفتوحًا**: تستقبل دالة `onMessage` (في `js/auth.js`) بيانات الإشعار وتعرضه مباشرة في الصفحة باستخدام نافذة `SweetAlert2`.

#### تحسينات مقترحة

*   **حذف التوكن عند تسجيل الخروج**:
    *   **الحالة**: تم التنفيذ.
    *   **التفاصيل**: دالة `logout()` في `js/auth.js` ترسل الآن طلب `DELETE` إلى `api/tokens.js` مع `user_key` الخاص بالمستخدم قبل مسح البيانات المحلية. هذا يضمن تنظيف قاعدة البيانات من التوكنات غير النشطة.
...
### 12. كيفية البدء (Getting Started)
