...
3.  **دالة التطبيق الشامل (`window.applyAppTranslations()`)**:
    ...
    *   تقوم بمسح الـ DOM وترجمة أي عنصر يحمل سمة `data-lkey` (للنصوص) أو `data-lkey-title` (للتلميحات) تلقائياً.
...
*   **ثبات الأيقونات**: عند ترجمة الأزرار التي تحتوي على أيقونات (`<i>`)، يجب وضع النص داخل وسم `<span>` يحمل `data-lkey` لحماية الأيقونة من المسح عند تحديث النص برمجياً.
...
## 6. كيفية البدء بترجمة موديول (صفحة) جديد
... (بقية الخطوات كما هي)
