...
*   **إنشاء حساب (خيار البائع)**: يدعم تكامل الخريطة وقفل النافذة العرضي. تمت إضافة خيار "تفعيل وضع البائع" عبر نافذة تفاعلية (Swal) لجمع بيانات التوصيل الذاتي (`isDelevred`) والحد الأدنى للطلبات (`limitPackage`) وحفظها دفعة واحدة مع بيانات المستخدم الأساسية.
*   **الملف الشخصي (تحديث الخيارات)**: يعتمد على "التحديث المحلي أولاً" عند اختيار موقع جديد أو تعديل بيانات البائع. يتم ضبط خيارات البائع عبر نافذة مخصصة تظهر عند الطلب، ولا يتم المزامنة مع السيرفر إلا عند الضغط على زر "حفظ التغييرات" لتأكيد كافة التعديلات.
