...
### ج. سجل الحاويات والسلوك التلقائي (`LOADER_REGISTRY`)
يُعد هذا السجل (Array) هو المحرك الأساسي لحالة واجهة المستخدم (UI State) وتاريخ التنقل:
*   **تتبع المسار**: يسجل ترتيب الحاويات المفتوحة، مما يسمح للتطبيق بمعرفة الحاوية السابقة بدقة (مثلاً للعودة من تعديل منتج إلى قائمة المنتجات).
*   **التحكم الذكي**: عند فتح حاوية جديدة، يتم إخفاء كافة الحاويات الأخرى المسجلة تلقائياً لمحاكاة نظام "التبويبات" (Tabs).
*   **تحسين الأداء**: إذا كانت الصفحة مسجلة مسبقاً، يتم إظهارها فوراً بدلاً من إعادة تحميلها (Caching DOM state)، ما لم يتم تفعيل خيار `reload: true`.
*   **اتخاذ القرارات البرمجية**: يُستخدم طول السجل (`LOADER_REGISTRY.length`) لتحديد السياق الحالي للمستخدم؛ فمثلاً، إذا كان السجل يحتوي على أكثر من عنصر، فهذا يعني أن المستخدم "داخل" عملية تصفح فرعية ويجب تفعيل "العودة" بدلاً من "إعادة التحميل" عند النقر على أيقونات التنقل العلوية.
