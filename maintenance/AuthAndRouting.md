...
*   **استمرارية التفضيلات**: عند استدعاء `logout()`، يقوم النظام بحفظ إعدادات اللغة (`app_language`) والمظهر (`theme`) بشكل مؤقت قبل مسح البيانات المحلّية، ثم يعيد تعيينها فوراً، لضمان استمرارية تجربة المستخدم باللغات والمظاهر المفضلة حتى بعد تسجيل الخروج.
...
## 6. نظام اختيار الفئة (Category Selection System)

يستخدم النظام نافذة مركزية (`CategoryModal`) لضمان اختيار التصنيف الصحيح قبل البدء في إضافة أي منتج أو خدمة.

### أ. آلية العمل المركزية
*   **دالة الاستدعاء**: `showAddProductModal()` هي المسؤولة عن بدء العملية.
*   **تخزين الحالة**: يتم حفظ الفئات المختارة (المعرف الرئيسي والفرعي) في `ProductStateManager`.
*   **التوجيه التلقائي**: بناءً على نوع الفئة المختارة، تقوم دالة `loadProductForm` بتحديد الصفحة المناسبة (`productAdd` للمنتجات العادية و `productAdd2` للخدمات).

### ب. منطق التخطي الذكي (Smart Skip Logic)
لتحسين تجربة المستخدم وتحقيق التكامل مع ميزة "التحميل الذكي"، تم تطوير المنطق ليتخطى نافذة الاختيار في الحالات التالية:
1.  **وجود مسودة نشطة**: إذا كانت حاوية الإضافة (`index-productAdd-container`) تحتوي بالفعل على صفحة محملة.
2.  **ثبات الفئات**: إذا كانت الفئات مختارة بالفعل في الذاكرة النشطة.
3.  **الاستمرارية**: يسمح هذا للمستخدم بالعودة لمتابعة عملية الإضافة فوراً دون تكرار خطوات الاختيار، مما يحافظ على تركيزه ويمنع فقدان البيانات المدخلة.
...
## 8. إدارة البيانات العالمية (Global Data Management)

لتحسين الأداء وتقليل طلبات الشبكة، تم اعتماد سياسة "الجلب لمرة واحدة" للبيانات الثابتة التي يتم استهلاكها في عدة واجهات:

### أ. قائمة الفئات (`window.appCategoriesList`)
*   **آلية التحميل**: يتم جلب ملف `shared/list.json` مرة واحدة فقط عند تحميل التطبيق (`DOMContentLoaded`) في ملف `index.js`.
*   **الدالة المساعدة**: توفر `fetchAppCategories()` في `js/globalVariable.js` وسيلة آمنة للوصول للبيانات؛ حيث تعيد البيانات من الذاكرة إذا كانت موجودة، أو تقوم بجلبها في حال عدم توفرها.
*   **الاستهلاك الموحد**: تستخدم كافة المكونات (البحث، الفئات، إدارة المنتجات، ومدير الحالة) هذا المتغير العام بدلاً من جلب الملف بشكل مستقل، مما يضمن اتساق البيانات وسرعة الاستجابة.
...
4.  **في البيانات**: عند الحاجة لبيانات التصنيفات، استخدم دائماً `window.appCategoriesList` أو `await fetchAppCategories()` لتجنب طلبات الشبكة الزائدة.
